...
lcmark.compile_template = function(template_str)
  ...
  if fail_pos then
    -- Since all newlines contain a line feed, we can count lines by counting
    -- line feeds and then adding 1
    local _, line_num = template_str:sub(1, fail_pos):gsub("\n", "")
    return nil, string.format("parse failure at line %d: '%s'", line_num + 1,
